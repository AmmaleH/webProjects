        ...
        <div class="order-box">
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              ...
              <div class="addr-add">
                <div class="icon-add" @click="openAddress"></div>
                <div>添加新地址</div>
              </div>
            </div>
          </div>
          <div class="item-good">
            ...
            <ul>
              <li>
                ...
                <div class="good-price">价格</div>
                <div class="good-total">合计价格</div>
              </li>
          ...
          <div class="btn-group">
            ...
            <a href="/#/order/pay" class="btn btn-large" @click="orderSubmit"
              >去结算</a
            >
          </div>
        </div>
  ...
  methods: {
    ...
    submitAddress() {
      ...
      if (userAction == 0 || userAction == 1) {
        ...
        var msg; // 用于验证收货的相关信息是否规范
        if (!receiverName) {
          msg = "请输入收货人姓名";
        } else if (!receiverMobile || !/(1)\d{10}/.test(receiverMobile)) {
          msg = "请输入正确格式的手机号";
        ...
        params = {
          // receiverName: checkedItem.receiverName,  // 未进行解构时的写法
          receiverName,
          receiverMobile,
          receiverProvince,
          receiverCity,
      ...
      //   this.closeModal();
      //   this.getAddressList(); // 防止多个地点同时登录，进行同一操作带来的并发问题。
      ...
      if (userAction == 0) {
        ...
        for (let key of Object.keys(params)) {
          this.list[this.checkIndex][key] = params[key];
        }
      } else {
        // 删除
        this.list.splice(this.checkIndex, 1);
      }
      this.closeModal();
      // this.getAddressList();
      this.$message.success("操作成功");
    ...
    getCartList() {
      ...
      this.axios.get("/user/cart").then((res) => {
        let list = res.cartProductVoList; // 购物车中所有的商品
        this.cartTotalPrice = res.cartTotalPrice; // 商品总金额
        this.cartList = list.filter((item) => item.productSelected); // 购物车中选中的商品
    ...
    orderSubmit() {
      ...
      //     });
      //   });
      this.$router.push({ path: "/order/pay", query: { orderNo: 1234 } });
    },
  },
};
</script>
